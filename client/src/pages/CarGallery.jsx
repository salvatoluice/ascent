...
const CarGallery = ({car}) => {
        ...
        return (
            <div className='absolute inset-0 bg-black text-white min-h-screen'>
                <div className='bg-black p-8 grid gap-4'>
                    <div>
                        <h2 className='text-2xl text-black'>{car.title} Photos</h2>
                        <button onClick={() => setShowAll(false)} className='fixed flex gap-1 right-12 top-8 py-2 px-4 rounded-xl shadow shadow-black bg-white'>
                            Close  
  ...
  return (
    <div className='relative'>
        ...
        <button onClick={() => setShowAll(true)} className='absolute bottom-2 right-2 py-2 px-4 bg-white rounded-xl border-black text-black shadow-shadow-md shadow-gray-500'>Show More Photos</button>
      </div>
  )
}
